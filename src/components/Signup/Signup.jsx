...
const Signup = ({ show, onClose, onLoginClick, signupFormData, setSignupFormData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post(process.env.NEXT_PUBLIC_API_URL + '/api/auth/register/user', signupFormData);
      if (response.data) {
          ...
          <div className="flex">
            ...
            <div className="hidden md:block w-[400px] bg-primary-600 p-8 text-white relative">
              <div className="h-full flex flex-col justify-between relative z-10">
                <div>
                  <h2 className="text-2xl font-bold mb-6">Join Exploreworld!</h2>
                  <p className="text-primary-100">Create an account to start your journey and unlock amazing travel experiences.</p>
                </div>
                <div className="space-y-6">
                  <div className="flex items-center gap-3">
                    <i className="fi fi-rr-map-marker text-xl"></i>
                    <p className="text-sm">Discover unique destinations</p>
                  </div>
                  <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm">Exclusive member benefits</p>
                  </div>
                </div>
              </div>
              {/* Decorative pattern */}
