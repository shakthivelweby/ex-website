...
import { downloadEventTicketPdf, getEventTicket } from "../../service";
...
export default function EventTicketClient({ bookingId }) {
  const router = useRouter();
  const normalizedBookingId = String(bookingId || "").trim();
  const { ready, loggedIn } = useClientAuth();

  const { data: ticket, isLoading, error, refetch, isFetching } = useQuery({
    queryKey: ["eventTicket", normalizedBookingId],
    queryFn: () => getEventTicket(normalizedBookingId),
    enabled:
      ready &&
      loggedIn &&
  ...
  if (!normalizedBookingId || normalizedBookingId === "undefined") {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-gray-100 px-4 dark:bg-gray-950">
        ...
        <button
          type="button"
          onClick={() => router.push("/my-bookings?tab=events")}
  ...
  return (
    <BookingTicket
      ...
      bookingType="event"
      onDownloadPdf={downloadEventTicketPdf}
      onClose={() => router.push("/my-bookings?tab=events")}
    />
  );
}
